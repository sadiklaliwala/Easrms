...
const DashboardPage = () => {
  ...
  if (isError || !response?.success) return <AppErrorState message="Failed to load dashboard data" />;

  const data = response.data;

  const metrics = [
    { title: 'Total Requests', count: data.totalRequests, icon: <DashboardIcon />, color: '#1976d2' },
    { title: 'Open', count: data.openCount, icon: <AssignmentIcon />, color: '#ed6c02' },
    { title: 'Pending Approval', count: data.pendingApprovalCount, icon: <PendingIcon />, color: '#9c27b0' },
    { title: 'Approved', count: data.approvedCount, icon: <CheckCircleIcon />, color: '#2e7d32' },
    { title: 'Assigned', count: data.assignedCount, icon: <BuildIcon />, color: '#0288d1' },
    { title: 'In Progress', count: data.inProgressCount, icon: <BuildIcon />, color: '#f57c00' },
    { title: 'Resolved', count: data.resolvedCount, icon: <CheckCircleIcon />, color: '#388e3c' },
    { title: 'Rejected', count: data.rejectedCount, icon: <CancelIcon />, color: '#d32f2f' },
    { title: 'Closed', count: data.closedCount, icon: <ArchiveIcon />, color: '#616161' },
  ];
    ...
    <Stack spacing={3}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          ...
          <AppPriorityChart data={data.byPriority} />
        </Grid>
