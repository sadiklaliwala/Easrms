...
import type { UserListDto, CreateUserDto, UpdateUserDto, UserQueryParams } from '../../../types/user.types';
...
const createSchema = Joi.object({
  fullName: Joi.string().min(2).required().messages({ 'string.empty': 'Full name is required' }),
  ...
  roleId: Joi.string().uuid().required().messages({ 'string.empty': 'Role is required' }),
...
// ─── Component ────────────────────────────────────────────────────────────────
const UserPage = () => {
  const [params, setParams] = useState<UserQueryParams>({ pageNumber: 1, pageSize: 10 });
  ...
  const managers = managersResponse?.data?.map((m) => ({ label: m.fullName, value: m.userId })) ?? [];
  ...
  const handleEdit = async (data: UpdateUserDto) => {
    if (!editUser) return;
    try {
      const res = await updateUser({ id: editUser.userId, body: data }).unwrap();
      if (res.success) {
  ...
  if (isError || !response?.success) return <AppErrorState message="Failed to load users" />;
    ...
    <Stack spacing={3}>
      ...
      >
        <Stack
          ...
          noValidate
        >
          <UserFormFields control={createControl} errors={createErrors} managers={managers} showPassword />
          <AppButton label={creating ? 'Creating...' : 'Create User'} type="submit" fullWidth disabled={creating} />
        </Stack>
      </AppModal>
      ...
      >
        <Stack
          ...
          noValidate
        >
          <UserFormFields control={editControl} errors={editErrors} managers={managers} showPassword={false} />
...
const UserFormFields = ({ control, errors, managers, showPassword }: UserFormFieldsProps) => {
    ...
    <>
      <Box>
        ...
        <Controller name="fullName" control={control} render={({ field }) => (
          <AppInput {...field} placeholder="Enter full name" fullWidth error={!!errors.fullName} />
        )} />
      ...
      <Box>
        ...
        <Controller name="email" control={control} render={({ field }) => (
          <AppInput {...field} placeholder="Enter email" type="email" fullWidth error={!!errors.email} />
        )} />
      ...
      {showPassword && (
        <Box>
          ...
          <Controller name="password" control={control} render={({ field }) => (
            <AppPasswordInput {...field} placeholder="Enter password" fullWidth error={!!errors.password} />
          )} />
      ...
      <Box>
        ...
        <Controller name="roleId" control={control} render={({ field }) => (
          <AppSelect {...field} options={roleOptions} fullWidth error={!!errors.roleId} />
        )} />
      ...
      <Box>
        ...
        <Controller name="managerId" control={control} render={({ field }) => (
          <AppSelect {...field} options={managers} fullWidth placeholder="Select manager (optional)" />
        )} />
      </Box>
    </>
  );
